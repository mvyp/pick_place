...
// Container for original & filtered data
  ...
  pcl::PCLPointCloud2* cloud_filtered = new pcl::PCLPointCloud2; //存储滤波后的数据格式
  ...
  pcl::PassThrough<pcl::PCLPointCloud2> px;	// 创建滤波器对象
	...
	px.setFilterLimits(-0.3, 0);		//设置Z字段过滤范围
	...
	px.setFilterLimits(-0.3, 0);		
	px.filter(*cloud_filtered);			

	px.setInputCloud(filptr);				
	px.setFilterFieldName("z");	
	px.setFilterLimits(0.3, 1.6);		
  px.filter(*cloud_filtered);		
